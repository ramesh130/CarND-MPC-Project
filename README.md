...
### The Model
We use the kinematic model for the vehicle in this project. It includes the vehicle's x and y coordinates, orientation angle (psi), and velocity, as well as the cross-track error and psi error (epsi). Actuator outputs are acceleration and delta (steering angle). The model combines the state and actuations from the previous timestep to calculate the state for the current timestep based on the equations below:
...
The Timestep Length (prediction horizon T) and Frequency were chosen empirically. The values that are finally chosen are - 
N = 10
dt = 0.1 
These values mean that the optimizer is considering a one-second duration in which to determine a corrective trajectory. Adjusting either N or dt (even by small amounts) often produced erratic behavior. Other combination of N and dt tried were - 20 / 0.05, 8 / 0.125, 6 / 0.15 etc.

### Polynomial Fitting and MPC Preprocessing
The way points are used to fit to a third order polynomial. The fitting process is in the car coordinates, so that the way points are transformed from the map coordinates to the car coordinates. This simplifies the process to fit a polynomial to the waypoints because the vehicle's x and y coordinates are now at the origin (0, 0) and the orientation angle is also zero.
